...
function visitNode(node, visitor) {
  ...
  if (node.type === 'MemberExpression') {
    return (
      ...
      (node.computed && visitNode(node.property, visitor))
    );
  }

  if (node.type === 'Property') {
    return (
      (node.computed && visitNode(node.key, visitor)) ||
...
function containsCaughtError(node, caughtErrorNames) {
  ...
  return visitNode(node, (current) => (
    current.type === 'Identifier' && caughtErrorNames.has(current.name)
  ));
}
...
module.exports = {
  meta: {
    ...
    messages: {
      defaultReturn: 'Error context is discarded by returning a default value. Wrap the original error or propagate it.',
      genericThrow: 'Error context is discarded by throwing a generic error. Wrap the original error with { cause: <error> } or use a custom error type.',
    },
    schema: [],
  },
  create(context) {
